...
class PolymarketClient:
    ...
    async def market_buy(self, token_id: str, shares: float, price: float = 0.99) -> dict | None:
        """Place a market-buy order using shares quantity at the given limit price."""
        if settings.trading.dry_run:
            log.warning("[DRY RUN] Would buy %.4f shares of %s @ $%.4f", shares, token_id, price)
            return None
    ...
    async def available_liquidity(self, token_id: str, max_price: float) -> tuple[float, float]:
        ...
        def _f(entry: Any, attr: str) -> float:
            return float(getattr(entry, attr) if hasattr(entry, attr) else entry[attr])

        eligible = [a for a in asks if _f(a, "price") <= max_price]
        if not eligible:
            return 0.0, 0.0
